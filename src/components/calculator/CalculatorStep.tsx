...
function StepWrapper({ id, title, helper, children }: StepWrapperProps) {
  return (
    <div className="calc-step">
      <h2 id={id} className="calc-question">
        {title}
      </h2>

      {helper && <p className="calc-helper">{helper}</p>}
...
export function StepProjectType({ onSelect }: StepProps<'web' | 'mobile' | 'platform'>) {
  return (
    <StepWrapper
...
export function StepComplexity({ onSelect }: StepProps<'simple' | 'medium' | 'complex'>) {
  return (
    <StepWrapper
      id="complexity-step"
...
export function StepScale({ onSelect }: StepProps<'mvp' | 'growth' | 'enterprise'>) {
  return (
    <StepWrapper
...
//step 4
export function StepTimeline({ onSelect }: StepProps<'flexible' | 'standard' | 'urgent'>) {
  return (
    <StepWrapper
